...
IMAGES_FOLDERS = ['D:/UNIVERSITA/Magistrale/SecondoAnno/Tesi/Datasets/MapillaryTrafficSignDetection/fully_annotated/training_set_1/images/',
...
DESTINATION_FOLDER = 'D:/UNIVERSITA/Magistrale/SecondoAnno/Tesi/Datasets/MapillaryTrafficSignDetection/direction_or_information/'


def main():
	total_files = 0 # total ammount of annotation files in the folder: 'ANNOTATION_FOLDER'
	images_to_take = [] # list of paths of files associated to images that contain 
						# 'direction-or-information' signs, which will be used to 
						# get those images
	...
	for folder in IMAGES_FOLDERS:
		for annotation_path in images_to_take:
			...
			try:
				...
				target_annotation = annotation_path.replace('fully_annotated/annotations/mtsd_v2_fully_annotated/annotations',
															'direction-or-information')
				shutil.copy(annotation_path, target_annotation) # copy annotation file 
																# ⚠️WARNING: this does not seem to work, even though
																# I checked the 'original' and 'target' paths and 
																# they both seem to be correct... 
				copied[original_img] = target
			except:
				pass
	...
	if len(copied) == len(images_to_take):
		...
		print(f" {len(images_to_take) - len(copied)} of {len(images_to_take)} images have not been copied; these are the file names:")
		# for name in images_to_take:
