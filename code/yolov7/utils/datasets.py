...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # invert content of bboxes only for direction-or-information signs (class "0")
            if random.random() < .5:
                xyxy_labels = np.copy(labels)
                xyxy_labels[:, 1:] = xywhn2xyxy(labels[:, 1:], img.shape[0], img.shape[1])
                for label in xyxy_labels:
                    if label[0] == 0 and random.random() < .5:
                        left  = int(label[1])
                        right = int(label[3])
                        up    = int(label[2])
                        down  = int(label[4])
                        img[up:down, left:right, :] = cv2.bitwise_not(img[up:down, left:right, :])
