...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    if type(coords) is list:
        ...
        for i in range(len(coords)): coords[i] /= gain
    else: 
...
def clip_coords(boxes, img_shape):
    ...
    if type(boxes) is list:
        if boxes[0] >= img_shape[1]: boxes[0] = img_shape[1] # x1
        if boxes[0] < 0: boxes[0] = 0                        # x1
        if boxes[1] >= img_shape[0]: boxes[1] = img_shape[0] # y1
        if boxes[1] < 0: boxes[1] = 0                        # y1
        if boxes[2] >= img_shape[1]: boxes[2] = img_shape[1] # x2
        if boxes[2] < 0: boxes[2] = 0                        # x2
        if boxes[3] >= img_shape[0]: boxes[3] = img_shape[0] # y2
        if boxes[3] < 0: boxes[3] = 0                        # y2
    else:
